"""Evaluate trained 20D nonlinear-case models via MMD vs time.

For every (seed, model) pair, loads
`outputs/20Dnonlinear<seed>_<Model>_dt<dt>/<ckpt>`,
...
def parse_dir_name(output_dir: str) -> Tuple[int, str, float]:
    """Parse '20Dnonlinear<seed>_<Model_name>_dt<dt>'."""
    base = os.path.basename(os.path.normpath(output_dir))
    parts = base.split("_")
    prefix = "20Dnonlinear"
    if len(parts) < 3 or not parts[0].startswith(prefix) or not parts[2].startswith("dt"):
        raise ValueError(
            f"Cannot parse output dir name '{base}'. "
            "Expected '20Dnonlinear<seed>_<Model_name>_dt<dt>'."
        )
    seed = int(parts[0][len(prefix):])
    model_name = parts[1]
...
DEFAULT_INIT_MIN = [-2.0] + [-1.0] * 19
DEFAULT_INIT_MAX = [2.0] + [1.0] * 19
...
def main():
    ...
    parser.add_argument("--outputs-root", default="outputs",
                        help="Root dir holding 20Dnonlinear<seed>_<Model>_dt<dt> folders.")
    ...
    parser.add_argument("--init-min", type=float, nargs="+", default=DEFAULT_INIT_MIN,
                        help="Lower corner of the IC sampling box (default: [-2] + [-1] * 19).")
    parser.add_argument("--init-max", type=float, nargs="+", default=DEFAULT_INIT_MAX,
                        help="Upper corner of the IC sampling box (default: [2] + [1] * 19).")
    ...
    parser.add_argument("--out-name", default=None,
                        help="Base name for npz/png. Default: 'mmd_summary_20D_dt<dt>'.")
    ...
    out_name = args.out_name or f"mmd_summary_20D_dt{dt_tag}"
    target_SDE = build_targets(kappa, dim=int(config0.dim))
    ...
    # All models at the same seed reuse the same inits_tgt/inits_mdl/BMs,
    # so MMD^2(t=0) is identical across HD2/Onsager per seed.
    seed_inputs = {}
    ...
    for mi, model_name in enumerate(args.models):
        for seed in args.seeds:
            tag = f"20Dnonlinear{seed}_{model_name}_dt{dt_tag}"
