...
from utils_sgld import load_data, generate_data, build_OnsagerNetHD2, get_filter_spec 
 
from logging import Logger 

def train_model(config: DictConfig, logger: Logger= None, batch_size=1, path: str ="test") -> None:
    """Main training routine

    Args:
        config (DictConfig): configuration object
    """
    subruntime_dir = os.path.join(path, f"batch_size_{batch_size}")
    os.makedirs(subruntime_dir, exist_ok=True)
    N=200000
    print(subruntime_dir)
    logger.info(f"Device {jax.devices()}...")

    cache_dir = "OriginalData/batch_size_{}".format(batch_size)
    if config.data.generate:
        logger.info(f"Generating data...")
        train_dataset = generate_data(total_seed=int(N*0.8), batch_size=batch_size)
        test_dataset = generate_data(total_seed=int(N*0.2), batch_size=batch_size)
...
@hydra.main(config_path="./config", config_name="main_sgld_res", version_base=None)
def main(config: DictConfig) -> None:
    runtime_dir = hydra.core.hydra_config.HydraConfig.get().runtime.output_dir
    logger = logging.getLogger(__name__) 
    for batch_size in [1, 2, 4, 8, 16, 32, 64]:
        train_model(config, logger, batch_size, runtime_dir) 
